...
public:
    ...
    int path_cost;
    int h_n;
    int compute_h_val(vector<vector<int>> temp) //Heuristic function  (No of misplaced tiles)
    {
        int ct = 0;
        for (int i = 0; i < 3; i++)
        {
    ...
    node(vector<vector<int>> ar, node *p, string ac, int path_cst, int h_val) //parametrized constructor
    {
    ...
    void successor() // Function to find child of a given node
    {
        ...
        while (!dir.empty()) // while loop to check for all the four directions
        {
...

            {
                ...
                for (int i = 0; i < 3; i++) // store thevalude of parent node in temp
                {
                    vector<int> temp2;
                    for (int j = 0; j < 3; j++)
                    {
                ...
                temp[xpos][ypos] = swap;
                int heuristic_val = compute_h_val(temp);
